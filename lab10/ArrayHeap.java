...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (getNode(parentIndex).myPriority > getNode(index).myPriority) {
    ...
    private void sink(int index) {
        ...
        if (getNode(minChildIndex) == null) {
            return;
        }
        if (getNode(minChildIndex).myPriority < getNode(index).myPriority) {
    ...
    public void changePriority(T item, double priority) {
        /* TODO: Your code here! */
        changeHelper(item, priority, 1);
    }

    private void changeHelper(T item, double priority, int index) {
        if (!inBounds(index)) {
            return;
        }
        if (item.equals(getNode(index).myItem)) {
            ...
            changeHelper(item, priority, leftIndex(index));
            changeHelper(item, priority, rightIndex(index));
        }
    }
